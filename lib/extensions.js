...
var self = {
	...
	insertOrUpdateRecord: function (pool, db, tableName, record, writeRecordCallback) {
		async.waterfall([
				function (callback) {
					pool.getConnection(function (err, connection) {
						callback(err, connection);
				...
				function (connection, callback) {
					var query = 'INSERT IGNORE INTO ??.?? SET ? ON DUPLICATE KEY UPDATE ?';
					connection.query(query, [db, tableName, record, record], function (err, results) {
						connection.release();
						callback(err, results);
					});
				}
